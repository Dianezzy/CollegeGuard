...
def generate_points_coordinate():
    ...
    print('AD边上平分的个数', str(num_split_AD))
...
def create_yichuxing_data():
    '''生成宜出行爬取的数据格式'''
    points_coordinate_result = generate_points_coordinate()
    ...
    for i, point in enumerate(points_coordinate_result):
        ...
        res = transCoordinateSystem.gcj02_to_wgs84(lng, lat)
        lng = res[0]
        lat = res[1]
        text += str(i) + ',' + str(lng) + ',' + str(lat) + '\n'

    with open(settings.xy_name, 'w') as f:
        f.write(text.strip())
    print('写入成功')
